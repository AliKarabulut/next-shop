  ...
  return (
    <>
      <div className="block md:hidden ml-auto" onClick={() => setIsSearchOpen(!isSearchOpen)}>
        {isSearchOpen ? <BsXLg className="text-2xl fill-black-70 cursor-pointer" /> : <BsSearch className="text-xl fill-black-70 cursor-pointer" />}
      </div>

      <form
        ref={searchRef}
        onSubmit={submitHandler}
        className={`max-md:absolute max-md:top-[3.5rem] z-50 left-0 flex items-center md:border-2 md:rounded-3xl border-yellow md:max-w-2xl w-full relative md:!flex ${
